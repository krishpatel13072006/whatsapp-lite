          ...
          >
            ...
            {status === 'sent' ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4 relative z-10">
                {[
                  { label: 'Name', type: 'text', key: 'name' as const, placeholder: 'John Doe' },
                  { label: 'Email', type: 'email', key: 'email' as const, placeholder: 'john@example.com' },
                ].map((field) => (
                  <div key={field.key}>
                ...
                <motion.button
                  ...
                  className={`w-full py-3 md:py-3.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition-colors ${
                    status === 'sending'
