  ...
  return (
    <section className="py-16 md:py-24 px-4 md:px-6 relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
                ...
                <AnimatePresence>
                  {isActive && (
                    <motion.div
                      ...
                      style={{ top: cardSize + 8, zIndex: 100 }}
                    >
        ...
        {/* Active feature detail below on mobile */}
        {isMobile && activeIndex !== null && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="glass-card p-4 mt-6 mx-auto max-w-sm"
          >
            <div className="flex items-center gap-3 mb-2">
              {React.createElement(FEATURES[activeIndex].icon, { size: 18, className: "text-primary" })}
              <h4 className="font-semibold text-foreground text-sm">{FEATURES[activeIndex].title}</h4>
            </div>
            <p className="text-muted-foreground text-xs">{FEATURES[activeIndex].desc}</p>
            <img
              src={FEATURES[activeIndex].img}
              alt={FEATURES[activeIndex].title}
              className="w-full h-32 object-cover rounded-lg mt-3 border border-border/30"
            />
          </motion.div>
        )}
      </div>
